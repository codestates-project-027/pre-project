...
import OverflowBlog from '../addimage/overflowblog.png'; 
...
const QuestionsPage = (isLogin) => {
  ...
  const menuArr= [
    { name: 'Newest' },{ name: 'Active' },{ name: 'Bountied' },{ name: 'Unanswered' },{ name: 'More' }
  ]
    ...
    <>
      <Div>
        <div className="main--wrapper">
          ...
          <div className="head">All Questions
                ...
                <TabMenu>
                 {menuArr.map((el,index)=>{
                   return (
                     <li
                      key={index}
                      className={currentTab === index? 'submenu active' : 'submenu'}
                  ...
                  <div className="body">{item.body}</div>
                    ...
                    <div className="tags">{item.tags} </div>
                       <div className="author--and--time">
                        <p className='author'>{item.author}</p> <p className='createdAt'> asked {item.createdAt}</p>
                      </div>
                    </div>
        ...
        <div className="sub--wrapper">
          <img className="overflowblog" src={OverflowBlog} /> {/* assets : overflowblog.png삭제 후 커밋하기 //merge 후 추가해서 commit*/  }
        </div> 
...
.tags{
  background-color: rgb(227,236,243); color: rgb(72,114,153);
  border-radius: 5px; padding: 4px; padding-left: 7px; padding-right: 7px;
  cursor: pointer;
}
.author--and--time{
  display: flex;
